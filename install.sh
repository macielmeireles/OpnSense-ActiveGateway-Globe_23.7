...
print_status() {
    ...
    check_and_download_files
    if echo "$tested_versions" | grep -q "$current_version" || cmp -s "gateways.widget.original.php" "gateways.widget.php"; then
        echo ""
        echo "Status: ✅ Safe - Tested Version or Same Files"
    else
        echo ""
